                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                        <li class="nav-item dropdown me-2" v-if="$isDemo == 1 && windowWidth > this.$mobileWidth">
                            <div class="d-flex gap-2 align-items-center mt-2"><span
                                    class="badge bg-danger">{{ __('demo_mode') }}</span></div>
                        </li>
                        ...
                        <div class="d-flex align-items-center" style="gap: 10px;">
                            <!-- Theme Toggle Button -->
                            <button type="button"
                                class="btn btn-outline-secondary btn-sm d-flex align-items-center justify-content-center p-2"
                                style="width: 40px; height: 40px;" title="Toggle Theme">
                                <label for="userTheme" style="cursor: pointer; margin: 0;">
                            ...
                            <!-- Clear Cache Button -->
                            <button type="button"
                                class="btn btn-outline-secondary btn-sm d-flex align-items-center justify-content-center p-2 position-relative"
                                style="width: 40px; height: 40px;" @click="popoverShow = true" id="confirmButton"
                                ref="confirmButton" title="Clear Cache">
                            ...
                            <b-popover target="confirmButton" triggers="click" :show.sync="popoverShow" placement="auto"
                                container="my-container" ref="popover">
                                <template #title>
                                    <div class="d-flex align-content-center justify-content-sm-between">
                                        <span> Confirmation</span>
                                        <button type="button" @click="popoverShow = false" class="close"
                                            aria-label="Close">
                                ...
                                <template #default>
                                    ...
                                    <hr />
                                    <div class="d-flex align-content-center justify-content-sm-between">
                                        <b-button @click="popoverShow = false" size="sm"
                            ...
                            <!-- Website Link Button -->
                            <button v-if="$websiteUrl" type="button"
                                class="btn btn-outline-secondary btn-sm d-flex align-items-center justify-content-center p-2"
                                style="width: 40px; height: 40px;" :href="$websiteUrl" target="_blank"
                                title="Visit Website">
                            ...
                            <div class="btn-group position-relative">
                                <button type="button"
                                    ...
                                    <span v-if="notifications_unread_count > 0"
                                        class="badge bg-success rounded-circle position-absolute translate-middle"
                                        style="bottom: 0px; left: 25px; font-size: 0.6em; padding: 2px 4px;">{{ notifications_unread_count }}</span>
                                </button>
                                    ...
                                    <li class="dropdown-item notification-item"
                                        ...
                                        <a class="d-flex align-items-center"
                                            ...
                                            <div class="notification-text ms-4">
                                                <p class="notification-title font-bold"> <router-link
                                                        :to="'/orders/view/' + notification.data.order_id">
                                                        {{ notification . data . text }}</router-link></p>
                                                <p class="notification-subtitle font-thin text-sm">
                                                    {{ changeDateTime(notification . created_at) }}</p>
                                            </div>
                                        </a>
                                    </li>
                                    <li v-if="notifications.length>0">
                                        <p class="text-center py-2 mb-0" v-if="isSellerRoute"><router-link
                                                to="/seller/notification_panel">See all notification</router-link></p>
                                        <p class="text-center py-2 mb-0" v-else><router-link
                                                to="/notification_panel">See all notification</router-link></p>
                                    </li>
                                    <li><a class="dropdown-item" v-if="notifications.length==0">No notification
                                            available</a></li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="dropdown" style="margin-left: 10px;">
                        <a href="#" data-bs-toggle="dropdown" aria-expanded="false">
                            <div class="user-menu d-flex">
                                <div class="user-name text-end me-3">
                                    <h6 class="mb-0 text-gray-600 text-capitalize">{{ user . username }}</h6>
                                    <p class="mb-0 text-sm text-gray-600 text-capitalize">{{ role }}</p>
                                </div>
                        ...
                        <ul class="dropdown-menu dropdown-menu-lg-end" aria-labelledby="dropdownMenuButton">
                            <li>
                                <h6 class="dropdown-header text-capitalize">Hello, {{ user . username }}!</h6>
                            </li>
    ...
    export default {
        ...
        data: function() {
            return {
                ...
                user: Auth.user,
                role: Role,
                profile_url: Role === 'Seller' ? Auth.user.seller.logo_url : Role === 'Delivery Boy' ? this
                    .$baseUrl + '/images/admin_logo.png' : this.$baseUrl + '/images/admin_logo.png',
                notifications: [],
        ...
        methods: {
            ...
            toggleTheme() {
                ...
                if (activeTheme === "theme-light" || activeTheme == "" || activeTheme == "undefined" || activeTheme ==
                    "null") {
